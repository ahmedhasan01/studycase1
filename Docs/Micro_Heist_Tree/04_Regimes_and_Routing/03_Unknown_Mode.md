...
## Re-entry Conditions (tighten-only)
  Return to normal routing ONLY when:
    ...
    - Regime/routing becomes stable for a persistence window,
...
### Frequency + failure handling (tighten-only)
  ...
  - Exits are always allowed; reduction management remains **reduce-first**.
