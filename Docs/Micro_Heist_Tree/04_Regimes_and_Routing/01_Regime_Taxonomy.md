...
## Output Contract (regime classifier)
  ...
  - `overlay_regime in {TREND, RANGE, CHOP_NOISE, HIGH_VOL_EXPANSION, LOW_VOL_COMPRESSION, SHOCK_DISLOCATION}` (Overlay minimal taxonomy; mapping below)`
...
## Overlay Alignment (Minimal Regime Taxonomy)
  ...
  ### overlay_regime (NON-BREAKING ADDITION)
  ...
  #### Mapping (conceptual)
    ...
    - `overlay_regime=CHOP_NOISE`          -> `core_regime=UNKNOWN` (default tighten-only / Unknown-Mode; exits/reductions allowed)
