...
## Operating Header
- Mission: Veto-capable gate deciding whether NEW entries are allowed right now.
- Hard constraints:
...
2) Verify Platform/Execution stability.
3) Verify Operator state.
4) Sanity-check market state (shock/abnormal behavior?).
5) Decide PASS / THROTTLE / BLOCK.
6) If THROTTLE/BLOCK: apply defaults immediately + record reason.
