...
## Operating Header
- Mission: Provide concise trading-only definitions used across the Micro Heist universe.
- Hard constraints:
...
- Canon note: If a term is used for gating (PASS/THROTTLE/BLOCK, Edge-positive, Unknown-Mode), it must align with:
  - Docs/Micro_Heist_Tree/01_Foundations/05_Decision_Glossary.md

## Procedure
1) When a term is missing or ambiguous: add [INBOX-REVIEW] + default stricter behavior.
2) Prefer linking canon over copying definitions.

## Terms (high-signal)
- Winning Bias: current favored side (Long/Short) determined by Bias System.
...
- Unknown-Mode: conflicts/low confidence; default BLOCK/THROTTLE entries; exits allowed.
...
<details>
<summary>Show legacy content (Definitions)</summary>
...
- Hard constraints (cannot override):
  ...
  - Do not duplicate canon elsewhere; other modules link here.
...
3) If the term is missing: add a short deterministic definition.
...
6) If any conflict is found with Core Invariants: tag [REVIEW-CONFLICT] and keep strict behavior.
