...
This file is an index/reference. Authoritative specs are the three route docs:
...
- This document is an index/reference; use the route docs above as authoritative specs.
...
CANONICAL START (Execution_OrderEngine)

Legacy execution notes (non-canonical):
- This block now points to the canonical OrderEngine spec in the `## OrderEngine` section below.
- Implementation/mechanics notes from prior drafts have been migrated into `## OrderEngine`; treat this block as a pointer only.
CANONICAL END (Execution_OrderEngine)
...
### A) Config surface (single source of truth; defaults in Parameter Registry)
...
- Allowed adaptives: size_mult ∈ [0.50, 1.10]; optional per-symbol size_mult_sym ∈ [0.50, 1.00] (SMALLCAP_THIN = 1.00)
...
- Dedup Map: duplicated sizing/caps/adaptives/telemetry rules consolidated into Shared conventions + pipeline stages A-D; execution-aware details kept in H as deltas.
- Precedence: canonical Risk_management rules above are authoritative; any legacy/garbled text outside the boundary is non-authoritative and should be treated only as pointers.
...
## Stream Eligibility
CANONICAL START (Stream_Eligibility)
...
This is the **interface contract** between Stream Eligibility and `RiskManager` (shared semantics; single source of truth lives in Heist). Stream Eligibility produces inputs that `RiskManager` may consume to **tighten only** (never loosen, never "invent" feasibility).
...
- `dq_score ∈ [0,1]` (WS data-quality scalar; used to tighten only)
...
- `RiskManager` and OrderEngine must not loosen Stream Eligibility outcomes and must not reimplement local orderbook reconstruction (partial depth is authoritative for this design).
...
- Schema (tighten-only):
  ...
  - `adapt_state ∈ {BOOTSTRAP, ADAPTING, READY, FROZEN}` per adaptive threshold set; if any `adaptive_required` item is not READY, force `entry_blocked=true` and `entry_mode=BLOCK` while continuing to compute/log; exits/reductions still allowed.
  ...
  - AggTrade fields (tighten-only): `aggtrade_state ∈ {BOOTSTRAP, ADAPTING, READY, FROZEN}`, `aggtrade_stale` (bool), `toxicity_score ∈ [0,1]` (if aggTrade disabled → 0 and source=DISABLED), `trade_intensity` (raw), `trade_intensity_pct` (optional), `signed_vol_imbalance ∈ [0,1]`
...
- Adaptive readiness state: `adapt_state ∈ {BOOTSTRAP, ADAPTING, READY, FROZEN}` for each adaptive threshold set.
...
### Cost & Slippage Budget (canonical formulas; tighten-only)
...
- Signed volume imbalance:
  - `v_buy = Σ qty` over window for buyer-initiated trades; `v_sell = Σ qty` for seller-initiated trades.
...
- Tape-to-feasible-size cap (tighten-only):
  - `trade_notional_window = Σ(price*qty)` over aggTrade window.
...
CANONICAL START (Candles_Bars_Tree_v4_MicroFirst_NoIntrabar)
...
- `percentile_rank(x, {x_{t-W+1..t}}) ∈ [0,100]` (time-series rank)
- `Pxx({values}) := percentile(values, xx)` (time-series percentile, per symbol)

##### 1.1.3 True Range & ATR (Shared)
- `TR_t := max(H_t - L_t, |H_t - C_{t-1}|, |L_t - C_{t-1}|)`
- `ATR_t := EMA(TR, n_ATR)` (canonical default; keep consistent everywhere)
...
- Candles/regime adaptive thresholds follow `adapt_state ∈ {BOOTSTRAP, ADAPTING, READY, FROZEN}` (same ladder as Stream Eligibility).
...
Define `bias_sign ∈ {-1,0,+1}`:
- `+1` supportive for LONG entries
- `0` neutral / unclear
- `-1` против LONG (prefer FLAT; treat as conflict for LONG entries when direction is required)
...
###### 1.3.4.2 Sub-minute Trades (< 1 minute)
- Sub-minute exits can happen; this does not change candles TF coverage (lowest candle TF remains M1).
- Candles handling for sub-minute intent:
  - Stay in **MICRO profile** (do not promote to MINI_SWING).
  - Use the **last closed M1 bar** (and last closed M5 bar) as the canonical gating state.
  - If the last closed M1 indicates unsafe (spike/trap), entries are blocked; otherwise entries may be eligible subject to the full MICRO chain.
...
##### Indicators (H1)
- Realized volatility (EWMA σ of log returns) → `vol_regime`, `vol_spike_flag`
...
- `sigma2_t := λ * sigma2_{t-1} + (1-λ) * r_t^2`
- `sigma_t := sqrt(sigma2_t)`

Half-life parameterization:
- `λ := exp( ln(0.5) / h )`  where `h` = half-life in bars

CHOP over n bars:
- `sumATR := Σ_{i=t-n+1..t} ATR_i`
...
##### Outputs (H1; minimal)
- `vol_regime ∈ {CALM, NORMAL, EXPANSION}`
- `market_regime ∈ {TREND_UP, TREND_DOWN, RANGE, TRANSITION, CHAOTIC_AVOID}`
...
##### Sanity checks (H1)
- σ rises in expansions and cools in calm periods (no “tiny bar spikes”).
...
- VWAP modes:
  ...
  - ROLLING_VWAP (windowed): `VWAP_roll_t := (Σ_{i=t-N+1..t} TP_i*V_i) / max(Σ_{i=t-N+1..t} V_i, EPS)`.
  - ANCHORED_VWAP: allowed only if an anchor policy exists; cumulative from anchor_id to t: `VWAP_anchor_t := (Σ_{i=anchor..t} TP_i*V_i) / max(Σ_{i=anchor..t} V_i, EPS)`.
- Config (no numbers invented; keep in INBOX if unset):
  - `candles_bars.m15.VWAP_MODE ∈ {SESSION_VWAP, ROLLING_VWAP, ANCHORED_VWAP}`
...
Session VWAP:
- `VWAP_t := (Σ_{i=session_start..t} TP_i * V_i) / max(Σ_{i=session_start..t} V_i, EPS)`

Canonical normalized distance (default):
- `dist_bps_t := 10_000 * (C_t - VWAP_t) / max(VWAP_t, EPS)`
...
- Donchian windows (bounded):
  - MICRO uses `N_micro := clamp(N_base_micro + ΔN(H1.market_regime), N_min_micro, N_max_micro)`
  - MINI  uses `N_mini  := clamp(N_base_mini  + ΔN(H1.market_regime), N_min_mini,  N_max_mini)`
  - TREND → smaller N; RANGE/CHOP → larger N
- Tiny-break q (bounded; regime-aware):
  - `q` stricter in CALM, looser in EXPANSION

##### Outputs (M5; minimal)
- `structure_valid ∈ {VALID, WEAK, INVALID}`
- `tiny_break_flag`
...
- Phase buckets (configurable; default fractions shown):
  - `PHASE_EARLY`: elapsed_frac ∈ [0.00, 0.33)
  - `PHASE_MID`:   elapsed_frac ∈ [0.33, 0.66)
  - `PHASE_LATE`:  elapsed_frac ∈ [0.66, 1.00]
...
- M5 Donchian bounds for MICRO and MINI: `N_min_micro/N_base_micro/N_max_micro` and `N_min_mini/N_base_mini/N_max_mini`, plus ΔN mapping from H1.market_regime (bootstrap = block until configured).
...
CANONICAL END (Candles_Bars_Tree_v4_MicroFirst_NoIntrabar)
...
- Stream selection (partial depth only): subscribe `<symbol>@depth{N}` with the active depth profile; **diff depth is excluded/disabled** (do not implement local book reconstruction for live gating).
- Depth profiles (deterministic defaults):
  - `LIVE_HOT` (default): `DEPTH_LEVELS_N=20`, `DEPTH_SPEED_MS=100`.
  - `LIVE_COLD` (only if required): `DEPTH_LEVELS_N=5`, `DEPTH_SPEED_MS=1000`.
  - Optional override: `DEPTH_SPEED_MS=50` is allowed only via explicit config and only when the strategy truly requires it; otherwise default stays 100ms.
  - `live_Cold` behavior: if subscribed to the cold profile, use it; if not, the system may remain subscribed to hot depth for continuity but must still behave as cold gating (no new entries / stricter caps) until cleared.
- Available Notional within N levels (tighten-only):
  - For bids (sell-side liquidity for exiting LONG / entering SELL): `avail_notional_bid_N = sum_{i=1..N} (bid_px_i * bid_qty_i)`.
  - For asks (buy-side liquidity for entering LONG): `avail_notional_ask_N = sum_{i=1..N} (ask_px_i * ask_qty_i)`.
  - Usage (tighten-only): constrain planned entry notional by a bounded fraction of visible notional on the relevant side, and feed `SMALLCAP_THIN` / `INSUFFICIENT_VISIBLE_DEPTH` / slip-cap enforcement.
  - This metric **does not trigger trades**; it only blocks or shrinks entries when visibility is thin.
...
- WS depth toolkit (merged, tighten-only; non-triggering):
  - L1-first notes: bookTicker gives bid1/ask1/qty; use for spread/tightness, L1 notional proxy, imbalance/microprice hints, shock pre-signal.
  - Depth data sources: WS partial `<symbol>@depth{5|10|20}` @100/1000ms; WS diff depth (requires snapshot/sequence); REST snapshot; WS API depth RPC.
  - Partial vs diff: partial for fast scalping impact proxies; diff+snapshot only if deep reconstruction is required (out of scope for live gating here).
  - Adaptive levels/speed guidance: default 20@100ms when active; 5/10/20 selectable if N=20 insufficient treat as thin; @1000ms (or pause) when only observing.
  - Inputs/invariants: bids/asks as strings; parse numeric; best prices at index 0; use EPS, tickSize/stepSize for precision compliance.
  - Derived metrics (telemetry/constraints only): mid/spread_bps/L1_notional/imbalance/microprice; depth_quote_bid/ask(N), min_depth_quote(N); depth_within_K_ticks; walk-the-book VWAP/slip_bps(Q); near_depth vs total_depth (near_ratio); multi-level imbalance; resilience (min_depth_med, depth_ratio, update rate, churn).
  - L3 eligibility core: VALID sample requires freshness, both sides present, sane L1; MIN_DEPTH_QUOTE default_source=WS_OBSERVED with bootstrap_value = P40 cross-sectional percentile over VALID (adaptive_required=true); tradable if VALID, not SHOCK, and min_depth_quote passes; optional slip_cap.
  - Liquidity shock override: liq_state ∈ {HEALTHY, THIN, SHOCK}; SHOCK on depth_ratio collapse/spread blowout/staleness; recover when spread normalizes and depth_ratio ≥ recovery threshold for RECOVERY_CONSEC.
  - Entries vs exits: THIN/SHOCK block new entries; exits may prioritize flattening; depth informs urgency.
  - Historical depth (ring buffer): useful for short-horizon baselines (10–60s), shock detection, micro health; not full-history accurate (partial depth misses intra-interval changes).
  - Operational cautions: depth can be spoofed; diff depth needs strict sequence; use EPS/finite guards; obey tickSize/stepSize.
  - Optional extensions (risk controls only, tighten-only; not alpha): OFI toxicity warning; impact slope (lambda) spike; depth shape/slope/convexity (HOLLOW_TOUCH); maker feasibility/queue-risk via churn/top_size_drop; resiliency via replenish_time; VPIN-based toxicity only if trades present; size scaling via Q_ENTRY_FRAC; optional reason codes (ENTRY_BLOCKED_TOXIC_FLOW/HOLLOW_TOUCH/MAKER_UNRELIABLE/VPIN etc.) consistent with existing reason style.
...
- WS history / ring-buffer reliability (tighten-only):
  - Historical inference (percentiles/baselines) is only trusted when stream continuity is good; gaps or resync degrade reliability.
  - Maintain `dq_score ∈ [0,1]` per symbol from continuity signals (e.g., gap rate, stale events, invalid payload rate, mismatch flags).
  - `dq_score` can only tighten: block NEW entries, shrink size, extend cooldown/quarantine; it must never loosen eligibility.
  - Implementation note: prefer a preallocated NumPy 2D array with a head index (ring semantics) for O(1) updates and stable memory (24/7 operation).
...
CANONICAL END (Stream_Eligibility)

## OrderEngine
CANONICAL START (OrderEngine)
...
- `order_intent` (from strategy/router after RiskManager sizing):
  - Fields: `intent_id`, `symbol`, `side` (BUY only for entry), `qty_target`, `notional_target` (optional), `urgency` ∈ {NORMAL, URGENT_EXIT}, `tif`, `allow_reduce_only` (bool).
- `feasibility_envelope` (from Stream Eligibility):
  - `entry_allowed`, `entry_mode` ∈ {BLOCK, PROBE_ONLY, MAKER_ONLY, TAKER_OK}
...
### Shared/Common (canonical mechanics)
- Scope/ownership: Strategy emits intents only; RiskManager owns sizing/caps and produces final order intent; Stream Eligibility + Bars readiness are tighten-only feasibility gates that OrderEngine must not loosen.
- Stress posture (hard invariant): under any WARMUP / STALE_FEED / BARS_WARMUP / BARS_STALE / SHOCK_LOCK / COOLDOWN_ACTIVE / quarantine states -> block NEW entries; exits/reductions allowed (still obey exchange filters/minNotional).
- Maker-first / taker-for-urgent-exits: default maker-first; taker (IOC/market) reserved for urgent exits or when maker cannot satisfy safety/risk constraints. If taker disallowed by caps/micro guards, fall back to passive exit and emit critical telemetry (do not open new risk).
- Determinism & replay: decisions deterministic given decision_ts_ms, config_version, thresholds_version_ts, stream states, bars readiness, account truth snapshot. Idempotency required (stable clientOrderId per intent; dedupe retries).
- Exchange feasibility & rounding: apply exchangeInfo filters (tickSize/stepSize/minNotional); never round up into higher risk; if rounding down violates filters -> block NEW entry (exits still attempted with best feasible qty).
- Optional adaptive execution patience: maker refresh cadence and maker->IOC flip timing obey bounded patience/hysteresis if enabled (default disabled until keys defined).

### Canonical Invariants (must-hold)
- Spot long-only: never open net short; SELL intents are EXIT/REDUCE only.
- Cancel-in-flight gate: never submit a replacement while CANCEL_REQUESTED is pending for the same intent/order; two-phase cancel/replace is non-bypassable.
- Single active working order per intent (default working_set=1) unless explicitly configured; cancel before replace when reducing working_set.
- Exchange/account state is authoritative on restart/reconcile; local state must conform.
- Envelope validity: never act on a Feasibility Envelope older than its TTL or with a superseded `envelope_version_id`.
- Exits/reductions are always allowed (subject to venue filters); when constraints conflict, choose the tighter action and still allow exits.
...
- Entities: Intent {symbol, side, qty, intent_type ∈ {ENTRY, EXIT, REDUCE}, urgency ∈ {NORMAL, URGENT}, price_policy, stop/TP params(optional), reason_context{primary_reason, secondary_reasons}, decision_ts_ms}; Order {clientOrderId, exchangeOrderId(optional), type, tif, price(optional), qty, status}.
...
### Crash Recovery & Orphan Policy (authoritative exchange state)
- On restart: fetch open orders, map to intents via clientOrderId/intent_id; if cannot map -> orphan -> cancel (unless mapped to known reduce-only exit). Reconcile fills/positions; exchange state wins. Reason codes: `OE_ORPHAN_ORDER_CANCELED`, `OE_REATTACHED_ORDER`, `OE_UNEXPECTED_FILL_RECONCILE`.
...
Integration Log
- Stream Eligibility deduped to the canonical boundary only; no stray or truncated sections remain.
- Small-cap segmentation reconciled: segmented percentiles are optional OFF-by-default; quota-only routing remains the default simpler mode.
- Placeholders removed/clarified: missing exchangeInfo handling (DROP vs KEEP + log), cancellation state wording, and other text corruptions.
- Spot wording corrected to LONG-only entries on spot; SHORT intents are EXIT/REDUCE only.
- Added tighten-only expectancy gate wording (expected_edge_bps vs worstcase_cost_bps + edge_buffer_bps).
- Candles/regime adaptive readiness aligned to the adapt_state ladder; adaptive_required not READY blocks new entries (exits allowed).
- Regime classifier made deterministic (direction/vol/clarity metrics, adaptive_required thresholds, hysteresis with K closes) and bucket names cleaned; Event/Shock re-enable formulas now explicit with conservative defaults.
- REST ticker eligibility now enforces adaptive readiness and deterministic handling for staleness/skew, small-N warmup, and fail_streak drop; exchangeInfo missing-handling and precision precedence made deterministic with DROP/KEEP+log.
- Global adaptive readiness rule added (default_source/bootstrap/min_samples/adapt_state, block entries when not READY); expectancy gate defaults to blocking until implemented; 1-20 horizon wording normalized.
- SPOT long-only wording made consistent; symmetric short handling moved to derivatives appendix (inactive for spot).
- Canonical OrderEngine boundary added; legacy execution block now a pointer with mechanics migrated into the canonical OrderEngine section.
- Example defaults hardened: event/shock re-enable constants made deterministic; MIN_DEPTH_QUOTE default made explicit; blocking bootstraps used where values are unset.
- Execution_OrderEngine canonical boundary deduped to pointer-only; all mechanics/state-machine/telemetry content lives in canonical OrderEngine.
- OrderEngine deduped: no unbounded duplicates remain; any unique facts are captured inside the canonical OrderEngine boundary.
- OrderEngine expanded: invariants, intent-to-plan compiler, slicing/pacing, maker escalation ladder, exit priority lane, venue feasibility, crash recovery, rate limit governor, telemetry/reason codes all added inside canonical boundary.
- Stream Eligibility: added cross-stream consistency gates, tape-to-feasible-size cap, toxicity ladder integration, and conservative defaults/config for consistency/aggTrade/tape caps; mode reducer clarified.
- OrderEngine: added concurrency limits, maker safety rails, rate-limit-aware planning, and chaos-test checklist.
- OrderEngine: integrated envelope consumption contract, telemetry output contract, deterministic replay/audit logging, and chaos/property harness mapping; reason codes extended.
- Stream Eligibility: added envelope TTL governance defaults and upstream telemetry tightening rules (ops_health/markout/execution quality).

Commonization Log
- Execution_OrderEngine canon consolidated to a single pointer-only boundary; canonical OrderEngine holds the implementation.
- OrderEngine implementation commonized: single bounded canonical section; no stray duplicates.
- Shared OrderEngine definitions (planning, pacing, ops governance) centralized under canonical boundary for reuse and consistency.
- Stream Eligibility shared definitions extended for aggTrade consistency/tape caps; toxicity ownership remains with SE (OrderEngine reports telemetry only).
- SE/OE shared field names aligned: envelope_version_id/TTL, telemetry fields, and tightening-only feedback contracts standardized.
